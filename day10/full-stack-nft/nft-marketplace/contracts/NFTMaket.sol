...
contract NFT is ReentrancyGuard {
    ...
    function createMarketItem(
        ...
        idToMarketItem[itemId] = MarketItem(
            ...
            tokenId,
            payable(msg.sender),
            payable(address(0)), //no one is the owner when nft created
            price,
            false
        );

        //transfer from owner to this deployed contract
        ...
        nonReentrant
    {
        ...
        //send token to the seller
        idToMarketItem[itemId].seller.transfer(msg.value);
        //send nft for the buyer from this contract deployed
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);

        //set the owner of nft is the buyer (sender)
        idToMarketItem[itemId].owner = payable(msg.sender);
        idToMarketItem[itemId].sold = true;

        //tang so luong item ban len 1, ma chua biet de lam gi
        _itemsSold.increment();

        //cai nay chac la charge fee ma ko biet dung ko
    ...
    //lay tat ca item ma minh da tao ra
